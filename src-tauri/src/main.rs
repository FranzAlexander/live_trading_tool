...
async fn app_start(
    ...
    tauri::async_runtime::spawn(kraken_websocket(ws_range_data, app_handle));

    Ok((sorted_range_bars, sorted_delta_bars))
}

async fn kraken_websocket(range_bars: Arc<Mutex<RangeData>>, app_handle: tauri::AppHandle) {
